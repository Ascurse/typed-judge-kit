...
def combine(a: dict[str, Answer]) -> tuple[float, str]:
    ...
    «ready → light_edit» (draft_lint_v2.combine), эта роль и восстановлена. Отдельный, более
    высокий порог под heavy_edit откалибровать нельзя: в стресс-наборе h37 нет ни одной
    фикстуры с заведомым overclaim, измеряются только ложные срабатывания (бид k55).
